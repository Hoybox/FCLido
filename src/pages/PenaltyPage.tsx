...
const PenaltyPage: React.FC = () => {
  ...
  const [ballPosition, setBallPosition] = useState({ top: '90%', left: '50%', transform: 'translate(-50%, -50%) scale(1)' });
  ...
  const LeaderboardDisplay = useMemo(
    () => (
      <div className="bg-gray-800/50 rounded-lg p-4 sm:p-6 border border-gray-700 w-full">
        <h3 className="text-xl sm:text-2xl font-bold text-[#fd6c9e] mb-4 text-center">CLASSEMENT</h3>
    ...
    return (
      <div className="container mx-auto px-4 flex flex-col items-center justify-center gap-8 text-center">
        ...
        <p className="text-lg text-gray-300 max-w-2xl">
          Marquez le plus de buts consécutifs possible pour atteindre le meilleur score. Choisissez une des 5 cibles pour tirer.
        </p>
        <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-gray-800/50 rounded-lg p-6 border border-gray-700 w-full flex flex-col items-center justify-center">
            <label htmlFor="playerName" className="block text-lg font-medium text-gray-300 mb-2">
              Votre pseudo
            </label>
            <input
              type="text"
            ...
            <p className="text-sm mt-4 text-gray-400">
              Record personnel : <span className="font-bold text-white">{highScore}</span>
            </p>
            <button
              onClick={() => setGameState('aiming')}
              className="mt-6 w-full max-w-xs justify-center py-3 px-4 rounded-md text-lg font-medium text-white bg-rose-600 hover:bg-rose-700 focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-rose-500 transition-transform hover:scale-105"
            >
              Commencer
            </button>
          </div>
  ...
  return (
    <div className="container mx-auto px-4 pb-20">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
              ...
              <div className="absolute inset-0">
                {targets.map(target => (
                  <button
                    ...
                    className="absolute w-1/5 h-1/4 bg-white/20 hover:bg-white/40 rounded-full border-2 border-dashed border-white/50 transform -translate-x-1/2 -translate-y-1/2 cursor-pointer"
                    style={{ top: targetPositions[target].top, left: targetPositions[target].left }}
                    aria-label={`Tirer vers ${target}`}
                  ></button>
                ))}
              </div>
            )}
              ...
              <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center text-center">
                {shotResult === 'goal' && (
                  <>
                    ...
                    <button onClick={nextRound} className="mt-4 bg-white text-black px-6 py-2 rounded-lg font-bold">
                      Tir suivant
                    </button>
                  </>
                )}
                {shotResult === 'saved' && (
                  <>
                    ...
                    <button onClick={playAgain} className="mt-4 bg-rose-600 text-white px-6 py-2 rounded-lg font-bold">
                      Rejouer
                    </button>
                  </>
                )}
        ...
        <div className="w-full lg:col-span-1">{LeaderboardDisplay}</div>
      </div>
    </div>
  );
};

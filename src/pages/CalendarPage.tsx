...
import { EventType, Role, CalendarEvent } from '../services/types';
...
const EventModal: React.FC<{
  ...
  onClose: () => void;
}> = ({ event, date, onSave, onClose }) => {
  const [formData, setFormData] = useState<Partial<CalendarEvent>>(
    event || { date: date || new Date(), type: EventType.Match }
  );
  ...
  return (
    ...
    >
      <div
        className="bg-gray-800 rounded-lg shadow-xl p-8 max-w-md w-full border border-rose-500"
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <select
            name="type"
            value={formData.type || EventType.Match}
            onChange={handleChange}
            className="w-full bg-gray-700 p-2 rounded"
          >
...
const CalendarPage: React.FC = () => {
  ...
  const getEventColor = (type: EventType): string => {
    switch (type) {
      case EventType.Match:
        return 'border-sky-400 bg-sky-400/10 hover:bg-sky-400/20';
      case EventType.Training:
        return 'border-green-400 bg-green-400/10 hover:bg-green-400/20';
      case EventType.Tournament:
        return 'border-orange-400 bg-orange-400/10 hover:bg-orange-400/20';
      case EventType.Break:
        return 'border-red-400 bg-red-400/10 hover:bg-red-400/20';
      case EventType.Extra:
        return 'border-pink-400 bg-pink-400/10 hover:bg-pink-400/20';
      default:
        return 'border-gray-400 bg-gray-400/10 hover:bg-gray-400/20';
    }
  };
  ...
  const handleShareEvent = async (event: CalendarEvent) => {
    ...
    if (navigator.share) {
      ...
      alert(
        `La fonction de partage n'est pas supportée sur votre navigateur. Copiez ce texte :\n\n${shareData.text}`
      );
    }
  };
    ...
    <div className="container mx-auto px-4 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-3xl font-bold text-[#fd6c9e] w-64 text-center">
        ...
        {isAdmin && (
          <button
            ...
            className="bg-rose-600 text-white px-4 py-2 rounded-md hover:bg-rose-700 flex items-center gap-2 transition-transform transform hover:scale-105"
          >
            <PlusIcon />
        ...
        <div className="grid grid-cols-7 grid-rows-5 gap-px">
          {dates.map((date, index) => {
            ...
            return (
              <div
                key={index}
                className={`relative min-h-[120px] p-2 bg-gray-800 transition-colors ${
                  isCurrentMonth ? 'hover:bg-gray-700/50' : 'bg-gray-900/50 text-gray-500'
                }`}
              >
                ...
                <div className="mt-1 space-y-1 overflow-y-auto max-h-24 text-xs">
                  {eventsOnDay.map(event => (
                    <div
                      key={event.id}
                      className={`group p-1 rounded text-left border-l-4 transition-colors relative ${getEventColor(
                        event.type
                      )}`}
                    >

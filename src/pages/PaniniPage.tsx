...
const CollectionView: React.FC<{
    ...
    const handleRevealCards = async () => {
        ...
        const generatedCards: PaniniCardData[] = [];
        try {
            ...
            generatedCards.push(...resolvedCards);
            ...
            setCollection(prev => {
                ...
                return { 
                    ...
                    albumSlots: newSlots, 
                    lastReveal: today,
                    cardInventory: [...(prev.cardInventory || []), ...generatedCards] 
                };
            });
            setRevealedCards(generatedCards);
        } catch (error) {
            console.error("An error occurred during card generation:", error);
    ...
    return (
        <div className="container mx-auto px-4 pb-20">
            ...
            <div className="mt-8">
                <h3 className="text-xl font-bold text-rose-500 mb-4">Cartes Récemment Révélées</h3>
                <div className="draw-pile w-full p-4 rounded-lg flex flex-wrap items-center justify-center gap-4">
                    {isLoading && <p className="text-gray-400">Génération des cartes...</p>}
                    {!isLoading && revealedCards.length === 0 && <p className="text-gray-400 w-full text-center">{canReveal ? "Révélez vos cartes du jour pour les voir ici." : "Vous avez déjà révélé vos cartes aujourd'hui."}</p>}
                    {!isLoading && revealedCards.map(card => (
                        <div key={card.id} className="w-28 animate-fade-in">

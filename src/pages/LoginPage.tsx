...
const ForgotPasswordModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsSending(false);
      setMessage(
        'Si un compte correspondant à cet identifiant existe, les instructions pour réinitialiser le mot de passe ont été envoyées.'
      );
    }, 2000);
  };
  ...
  return (
    <div
      className="modal-overlay fixed inset-0 bg-black/70 flex items-center justify-center backdrop-blur-sm z-50"
      onClick={onClose}
    >
      <div
      ...
      >
        ...
        <p className="text-gray-400 mb-4 text-sm">
          Entrez votre identifiant pour recevoir les instructions de réinitialisation.
        </p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="loginId_reset" className="block text-sm font-medium text-gray-300">
              Identifiant
            </label>
            <input
              type="text"
          ...
          {message && (
            <p className={`text-sm ${isSending ? 'text-yellow-400' : 'text-green-400'}`}>{message}</p>
          )}
          <div className="flex justify-end gap-4 pt-4">
            ...
            <button
              type="submit"
              className="px-4 py-2 rounded bg-rose-600 hover:bg-rose-700 disabled:bg-gray-500"
              disabled={isSending}
            >
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                <button
                  ...
                  aria-label={
                    isPasswordVisible ? 'Cacher le mot de passe' : 'Afficher le mot de passe'
                  }
                >

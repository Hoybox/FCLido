...
import { ClubInfo, Role } from '../services/types';
import { PencilIcon, TrashIcon } from '../components/icons/Icons';
import Logo from '../components/Logo';
import { useSite } from '../hooks/useSite';
import { useAuth } from '../contexts/useAuth';

const EditableField: React.FC<{ label: string; value: string; onSave: (newValue: string) => void; isAdmin: boolean }> = ({
  label,
  value,
  onSave,
  isAdmin,
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="bg-gray-700 text-white p-1 rounded-md text-right w-32"
          />
  ...
  return (
    <div className="flex justify-between group">
      ...
      <div className="flex items-center gap-2">
        ...
        {isAdmin && (
          <button onClick={() => setIsEditing(true)} className="opacity-0 group-hover:opacity-100">
            <PencilIcon />
          </button>
        )}
      </div>
...
const ClubPage: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-8 pb-20">
      {/* Colonne de gauche - Staff & Palmarès */}
      ...
      {/* Colonne de droite - Présentation & Actualités */}
